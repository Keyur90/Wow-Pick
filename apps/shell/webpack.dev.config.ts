...
const chromeProfileDirectory = findCacheDir({ name: 'google-profile' });
...
const config: webpack.Configuration = {
  ...
  entry: './src/main.tsx',
  target: 'browserslist',
  cache: { type: 'filesystem', name: 'shell' },
  ...
  devServer: {
    ...
    firewall: false,
    https: true,
    host: 'rf2.ecfk8saas.dev.wx-d.net',
    openPage: 'https://rf2.ecfk8saas.dev.wx-d.net:4000',
    open: {
      app: [
        open.apps.chrome, // Chrome browser app name depend on OS, 'Google Chrome' on macOS, 'google-chrome' on Linux and 'chrome' on Windows
        '--disable-web-security', // to bypass CORS/Cookie restrictions rules
        '--ignore-certificate-errors',
        '--user-data-dir=' + chromeProfileDirectory, // findCacheDir locates node_modules folder and add cache under .cache folder
      ],
    },
    hot: true,
  ...
  module: {
    rules: [
      // JavaScript: Use Babel to transpile JavaScript/Typescript files
      {
        ...
        use: {
          loader: 'babel-loader',
          options: { extends: babelConfig, plugins: ['react-refresh/babel'] },
        },
      },
  ...
  plugins: [
    ...
    // Enable hot module reload with react
